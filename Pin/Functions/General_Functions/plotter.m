...
%save_plot(figure(n),sprintf('.\\Runs\\error_dist\\Run__Gauss\\rel_abs_error_XY_%i__%i_rays__surf',num_zones(1),N),1)
%save_plot(figure(n),sprintf('.\\Runs\\error_dist\\Run__Gauss\\rel_abs_error_XY_%i__%i_rays__int',num_zones(1),N),1)
n = n + 1;
...
clf(n)
set(gcf,'Position', figsize,'DefaultAxesFontSize',fontsize)
semilogy(X(1:end-1,1),rel_error(1:end-1,1),'x-')
title('Rel Error',' ','Fontsize',18)
xlabel('x [cm]')
ylabel('Rel Absorp Error')
extra_axes(num_zones,1)
save_plot(figure(n),sprintf('%s\\rel_abs_error_j1',folder),savePlots)
n = n+1;
